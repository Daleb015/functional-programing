...
public class HighOrderFunctions {
  ...
  public static void main(String[] args) {
    IFactoryHighOrders<Integer> createFactory = createFactory(
      () -> Math.random() * 100,
      r -> r.intValue()
    );
  ...
  public static <T, R> IFactoryHighOrders<R> createFactory(
    IProducrHighOrders<T> producer,
    IConfiguratorHighOrders<T, R> configurator
  ) {
    return () -> {
