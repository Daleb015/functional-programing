...
public class TreeFun<E extends Comparable<E>> {
  ...
  public TreeFun(TreeFun<E> left, E value, TreeFun<E> right) {
    this.value = value;
    this.left = left;
    this.right = right;
  ...
  private TreeFun<E> insert(E newValue) {
    return isEmpty()
      ? new TreeFun<E>(NIL, newValue, NIL)
      : newValue.compareTo(this.value) < 0
        ? new TreeFun<E>(left.insert(newValue), this.value, this.right)
        : newValue.compareTo(this.value) > 0
          ? new TreeFun<E>(this.left, this.value, right.insert(newValue))
  ...
  private TreeFun<E> merge(TreeFun<E> right) {
    if (this.isEmpty()) return right; else if (
      right.isEmpty()
    ) return this; else return new TreeFun<E>(this.left.merge(this.right), this.value, right);
  }

  public boolean isMember(E value) {
    if (this.value != null) {
      // if the parameter is smaller than the value
      if (value.compareTo(this.value) < 0) return left.isMember(value);
      // if the parameter is greater than the value
      else if (value.compareTo(this.value) > 0) return right.isMember(value); else return true;
    } else {
      return false;
    }
  }
